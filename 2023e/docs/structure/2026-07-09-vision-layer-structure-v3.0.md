...
High-frequency path:
  red/green laser (LAB halo-guided core) + tracker ROI
...
**从 HSV 迁移到 LAB halo-guided core**:

- `color_space`: "HSV" → "LAB_HALO_GUIDED"
- A-channel 红绿分离替代 HSV hue 多区间
- L-channel 白心搜索替代 V-channel bright core
- halo-mask-restricted 搜索避免背景干扰

### 16.2 新增参数

`LaserDetectionParams` 新增 LAB 相关参数：

```text
a_threshold: int                    # A-channel 阈值
halo_blob_min_area_px: float       # halo 最小面积
halo_blob_max_area_px: float       # halo 最大面积
core_dilate_kernel_size: int       # 白心搜索膨胀核
core_l_percentile: int             # L 通道百分位
...
保留 HSV 分支，支持通过 `color_space` 参数切换：
- `color_space="LAB_HALO_GUIDED"` → LAB 算法（推荐）
- `color_space="HSV"` → HSV 算法（fallback）
