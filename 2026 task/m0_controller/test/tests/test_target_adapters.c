...
/* IR-tracker fake data */
static ir_uart_sensor_status_t g_fake_ir_process_status = IR_UART_SENSOR_STATUS_OK;
static bool g_fake_ir_analog_valid = true;
static uint16_t g_fake_ir_raw[IR_UART_SENSOR_COUNT] = {0};
...
/* Override IR-tracker driver functions (real ir_uart_sensor.c needs MSPM0 HW) */
ir_uart_sensor_status_t IrUartSensor_Process(void) {
    return g_fake_ir_process_status;
}

bool IrUartSensor_GetAnalog(uint16_t values[IR_UART_SENSOR_COUNT]) {
    uint8_t i;
    if (!g_fake_ir_analog_valid || values == NULL) {
        return false;
    }
    for (i = 0U; i < IR_UART_SENSOR_COUNT; ++i) {
        values[i] = g_fake_ir_raw[i];
    }
    return true;
}
 ...
 *
 * Runs BEFORE test_sensor_hal_ir_polarity: ReadIr() caches a static g_ir_valid
 * flag that, once set true by a successful frame, never resets. The "no frame
 * -> SD_ERR_READ" branch is only reachable while g_ir_valid is still false, so
 * this case must be exercised before any successful read.
 * ============================================================================ */
...
static void test_sensor_hal_ir_failure(void) {
    ...
    /* No valid frame yet -> SD_ERR_READ (warm-up / no-frame path) */
    g_fake_ir_process_status = IR_UART_SENSOR_STATUS_NO_FRAME;
    g_fake_ir_analog_valid = false;
    sd_status_t status = hal->read_ir(&mask, values);
    assert(status == SD_ERR_READ);

    /* NULL active_mask argument */
    g_fake_ir_process_status = IR_UART_SENSOR_STATUS_OK;
    status = hal->read_ir(NULL, values);
    assert(status == SD_ERR_INVALID_ARGUMENT);

    /* NULL values argument */
    status = hal->read_ir(&mask, NULL);
    assert(status == SD_ERR_INVALID_ARGUMENT);
...
/* ============================================================================
 * Test: Sensor HAL - IR Analog Grading and Mask
 * ============================================================================ */

static void test_sensor_hal_ir_polarity(void) {
    printf("TEST: Sensor HAL IR analog grading and mask\n");

    uint8_t i;
    const sensor_hal_t *hal = SensorAdapter_GetHal();
    assert(hal != NULL);
    assert(hal->read_ir != NULL);

    /* Channel 0 fully on line (raw 0 -> reflectance 1.0); others off (raw 4096 -> 0.0) */
    g_fake_ir_process_status = IR_UART_SENSOR_STATUS_OK;
    g_fake_ir_analog_valid = true;
    for (i = 0U; i < IR_UART_SENSOR_COUNT; ++i) {
        g_fake_ir_raw[i] = (i == 0U) ? 0U : 4096U;
    }
    ...
    for (i = 1U; i < SD_IR_CHANNEL_COUNT; ++i) {
        assert(values[i] == 0.0f);
    }

    printf("  PASS: IR analog graded, channel 0 active\n");
}
