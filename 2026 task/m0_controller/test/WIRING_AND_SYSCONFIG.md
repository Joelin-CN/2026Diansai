...
| I2C0_SDA | PA0 | H1-1 | PINCM1 / I2C0_SDA | 100 kHz；仅 3.3 V OLED，MCP23017 已移除（循迹改用 UART1） |
...
循迹采用 8 路红外循迹模块，经 UART1 以模拟分级（A2）方式上报 8 路反射强度，不再使用 MCP23017 I2C 扩展器。UART1 已在 SysConfig 配置，不新增 MCU，也不修改 SysConfig。

| 项 | 配置 |
|---|---|
| UART | UART1，PA8(TX) / PA9(RX)，115200 8N1 |
| 供电 | 模块 5 V 供电；MCU UART 为 3.3 V 逻辑，需确认模块 TX 输出电平，必要时做电平转换或分压 |
| 请求帧 | MCU 上电后发送 `$0,1,0#` 请求模拟流（`IrUartSensor_RequestAnalogMode`） |
| 数据帧 | `$A,x1:v1,x2:v2,...,x8:v8#`，8 路十进制模拟量（模块输出 0–4095；驱动解析接受 0–65535） |
| ISR | `UART1_IRQHandler`（ir_uart_sensor.c），优先级 2（低于 500 Hz 控制定时器 TIMG0 的优先级 1），在 RX 中断排空 RX FIFO → `IrUartSensor_RxByte` 拼帧，不调 FreeRTOS API |
| 任务侧 | `IrUartSensor_Process` 解析一帧、`IrUartSensor_GetAnalog` 取 8 路原值；`sensor_adapter.c` 的 `ReadIr` 归一化 `1 - raw/4096`（raw=0→1.0 最强反射/压线，raw=4096→0），阈值 0.5 生成 `active_mask`，50 Hz 非阻塞 |
| 预热 | 上电约 20 s 稳定期，首帧到达前 `read_ir` 返回 `SD_ERR_READ`，行为规划器走降级/丢线状态 |

MCP23017 已从控制应用初始化移除；`line_sensor.c` 中仍保留对 `MCP23017_ReadInputs` 的死代码引用，未进入循迹数据通路。
