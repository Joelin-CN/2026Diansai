...
static void display_raw_sensors(void) {
    ...
    printf("传感器布局（从车头看）:\n");
    printf("左侧 ←                    中心                     → 右侧\n");
    printf("-39.86  -28.47  -17.08  -5.69   5.69   17.08   28.47   39.86 (mm)\n");
    printf("  [7]     [6]     [5]     [4]     [3]    [2]     [1]     [0]  (索引)\n");
    printf("\n");
    printf("⚠️  注意: 索引0在最右侧(+39.86mm)，索引7在最左侧(-39.86mm)\n\n");
    ...
    while (PlatformTime_GetMillis() - start_time < TEST_DURATION_MS) {
        ...
        if (now - last_sample >= SAMPLE_INTERVAL_MS) {
            ...
            if (IrUartSensor_GetAnalog(ir_raw)) {
                ...
                // IR权重（根据config.c中的配置）
                // 注意: 索引0 = 最右侧(+39.86mm), 索引7 = 最左侧(-39.86mm)
                const float ir_weights[8] = {
                    3.9861f,   // 索引0: X=+39.86mm (最右)
                    2.8472f,   // 索引1: X=+28.47mm
                    1.7083f,   // 索引2: X=+17.08mm
                    0.5694f,   // 索引3: X=+5.69mm
                    -0.5694f,  // 索引4: X=-5.69mm
                    -1.7083f,  // 索引5: X=-17.08mm
                    -2.8472f,  // 索引6: X=-28.47mm
                    -3.9861f   // 索引7: X=-39.86mm (最左)
                };
...
void IrSensorCalibration_Run(void) {
    ...
    printf("📐 传感器配置信息:\n");
    printf("  传感器数量: 8\n");
    printf("  Y坐标: 132.1 mm (车头前方)\n");
    printf("  X范围: -39.86 到 +39.86 mm\n");
    printf("  总宽度: 79.72 mm\n\n");
