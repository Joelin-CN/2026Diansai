...
void sd_config_reset_defaults(void) {
    // IR传感器横向位置权重 (代码坐标系: 前=+X, 左=+Y)
    // 物理 X_phys(mm): [+5.69, +17.08, +28.47, +39.86, -5.69, -17.08, -28.47, -39.86]
    // 代码帧 Y(mm):    [-5.69, -17.08, -28.47, -39.86, +5.69, +17.08, +28.47, +39.86]
    // 权重 = code_Y / 10mm  (右侧为负, 左侧为正, 越外侧绝对值越大)
    ...
    g_sens_decision_config.vehicle.wheel_track_m = 0.115f;  // 修正: 115mm实测值
    ...
    for (index = 0U; index < SD_ENCODER_COUNT; ++index) {
        ...
        encoder->wheel_radius_m = 0.033f;  // 修正：与motion_config.h保持一致
    ...
    g_sens_decision_config.perception.position.x_m = 0.1321f;   // X方向: 前方132.1mm (代码帧 前=+X)
    g_sens_decision_config.perception.position.y_m = 0.0f;      // Y方向: 横向居中
    g_sens_decision_config.perception.position.z_m = -0.02f;    // Z方向: 保持原值
