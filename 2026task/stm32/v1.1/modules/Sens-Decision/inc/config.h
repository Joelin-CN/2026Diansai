...
typedef struct {
    bool active_high;  // true: 黑线ADC高于背景；false: 黑线ADC低于背景
    float weights[SD_IR_CHANNEL_COUNT];
    sd_position_t position;
    float heading_filter_alpha;
    float white_reference[SD_IR_CHANNEL_COUNT];  // 校准参考端ADC（名称为历史兼容）
    float black_strength_threshold;              // 参考端到判决边界的ADC差值
