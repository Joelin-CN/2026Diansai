...
static float line_strength_for_channel(const sd_perception_config_t *config,
                                       const ir_array_data_t *ir_data,
                                       uint8_t channel) {
    float decision_level =
        config->white_reference[channel] - config->black_strength_threshold;
    float strength = config->active_high
                         ? ir_data->values[channel] - decision_level
                         : decision_level - ir_data->values[channel];
...
void perception_debug_print(const ir_array_data_t *ir_data,
    ...
    // 越过判决边界后的黑线置信度
    printf("Line Strength: ");
    uint8_t active_count = 0;
    for (uint8_t i = 0; i < SD_IR_CHANNEL_COUNT; i++) {
        float strength = line_strength_for_channel(config, ir_data, i);
    ...
    for (int8_t i = 7; i >= 0; i--) {
        printf("%c", ((result->active_mask & (uint16_t)(1U << i)) != 0U) ? '1' : '0');
        if (i == 4) printf(" ");
    }
    printf(")\r\n");
    ...
    printf("Polarity:      black ADC %s background\r\n",
           config->active_high ? "HIGHER than" : "LOWER than");
...
bool perception_debug_selfcheck(void) {
    ...
    // 检查1: 校准参考端ADC
    printf("[1] Calibration Reference Values:\r\n");
    for (uint8_t i = 0; i < SD_IR_CHANNEL_COUNT; i++) {
        float ref = config->white_reference[i];
        if (ref < 1.0f || ref > 4095.0f) {
            printf("  ❌ Channel %u: %.1f (out of ADC range 1-4095)\r\n", i, ref);
            passed = false;
        } else {
            printf("  ✓ Channel %u: %.1f\r\n", i, ref);
        }
    ...
    if (threshold < 1.0f || threshold > 4095.0f) {
        printf("  ❌ Threshold: %.1f (out of ADC range 1-4095)\r\n", threshold);
        passed = false;
    } else {
        printf("  ✓ Threshold: %.1f\r\n", threshold);
    }
