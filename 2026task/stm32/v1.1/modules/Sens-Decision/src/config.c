...
void sd_config_reset_defaults(void) {
     ...
     */
    /*
     * 2026-07-31 bench log: a positive forward PWM produced increasing raw
     * counts on both B(left) and C(right).  The previous {-1, -1} converted
     * both forward wheel speeds to negative values, so both speed PI loops
     * saturated at the same positive PWM and erased all steering authority.
     */
    ...
    /*
     * 2026-07-30实测：本车循迹板黑线ADC约1136、白底ADC约282，
     * 因而黑线为高电平侧。perception.c必须按此标志选择判决极性。
     */
    ...
    /* 宽黑线边缘切换时抑制误差微分尖峰，数值越大滤波越强。 */
    g_sens_decision_config.perception.heading_filter_alpha = 0.75f;

    /**
     * 校准参考端ADC（字段名white_reference为历史兼容）
     *
     * @category A: 物理测量参数（需要校准）
     *
     * @value 1136.0f（当前反向传感器的黑线参考端实测值）
     *
     * @origin 2026-07-30实测
     *   - 白色背景平均约281.6
     *   - 黑色线条平均约1136.1
     ...
     *   3. 自动采样100次并更新此数组
     *
     * @algorithm 当前active_high=true:
     *   decision_level = reference - threshold = 1136 - 427 = 709
     *   line_strength = raw - decision_level
     *   - 白色区域约282: line_strength < 0，不激活
     *   - 黑线区域约1136: line_strength > 0，激活
     *
     * @warnings
     *   - 光照条件变化时需要重新校准
     *   - 传感器老化或更换后需要重新校准
     ...
     *   - 阈值过高: 黑线检测不到（灵敏度不足）
     *
     *   当前实车建议从427附近小幅调整，并用mask确认白底不命中、
     *   黑线命中后再落地运行。
     *
     * @warnings
     *   - 必须在校准白色参考值后再校准此阈值
