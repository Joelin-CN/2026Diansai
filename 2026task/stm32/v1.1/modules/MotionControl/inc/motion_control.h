...
typedef struct {
    ...
    /* ---- IMU yaw-rate inner loop ---- */
    float gyro_z_radps;             /**< Latest body yaw rate, CCW positive */
    bool gyro_z_valid;              /**< True while the latest IMU sample is valid */
    float yaw_rate_error_radps;     /**< omega_cmd - gyro_z */
    float steering_pwm;             /**< Final differential half-PWM */
    float wheel_pi_steering_pwm;    /**< Filtered differential wheel-PI trim */
...
/**
 * @brief Supply the latest ICM42688 yaw-rate sample to the steering inner loop.
 *
 * When valid is false the mixer keeps operating from yaw-rate feed-forward,
 * so loss of the IMU degrades control authority but does not stop line
 * following.
 */
