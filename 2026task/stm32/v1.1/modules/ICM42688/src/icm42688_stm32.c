...
static uint8_t _read_reg(uint8_t reg) {
    uint8_t address = (uint8_t)(reg | 0x80);  // 读操作：最高位置1
    uint8_t value = 0U;

    HAL_GPIO_WritePin(IMU_CS_GPIO_Port, IMU_CS_Pin, GPIO_PIN_RESET);
    /*
     * Preserve the transaction shape of the hardware-verified
     * Gyro20260123 driver: address phase first, then one receive phase while
     * CS stays low.  Do not replace this with a two-byte TransmitReceive.
     */
    (void)HAL_SPI_Transmit(&hspi2, &address, 1, 100);
    (void)HAL_SPI_Receive(&hspi2, &value, 1, 100);
...
static void _init(void) {
    /* SPI2 is initialized by CubeMX; force the verified idle CS state. */
    HAL_GPIO_WritePin(IMU_CS_GPIO_Port, IMU_CS_Pin, GPIO_PIN_SET);
}
