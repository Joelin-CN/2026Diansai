...
icm42688_status_t icm42688_init(void)
{
    ...
    p_comm->init();
    p_system->delay_ms(100);

    /*
     * Verify the sensor ID with retries.  The IMU is powered independently
     * from an ST-Link reset, so a warm MCU restart or a slow rail ramp can
     * make the first SPI transaction invalid even though the wiring is good.
     */
    id = 0U;
    ...
    id_matches = (id == ICM42688_ID);

    /*
     * Continue configuring even if WHO_AM_I was not returned correctly.
     * The original v1.0 driver allowed subsequent data reads in this case and
     * the field hardware has demonstrated that behavior.  The application
     * will accept this compatibility path only after plausible live
     * accelerometer samples are observed.
     */
    ...
    initialized = true;
    return id_matches ? ICM42688_STATUS_OK : ICM42688_STATUS_BAD_ID;
}
...
icm42688_status_t icm42688_read(icm42688_data_t *data)
{
    ...
    /*
     * HAL_SPI can return HAL_OK even when MISO is stuck low/high.  Never mark
     * such a transport frame as a valid IMU sample.
     */
