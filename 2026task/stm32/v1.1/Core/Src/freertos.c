...
#ifndef IMU_DIAGNOSTIC_PROFILE
#define IMU_DIAGNOSTIC_PROFILE 0
#endif
...
/* OLED uses blocking I2C transfers, so it must not run in the 500 Hz loop. */
...
void StartDisplayTask(void *argument)
{
  ...
  /* The controller initializes OLED first; this call is a no-op until ready. */
  for (;;)
  {
    CompetitionTasks_DisplayCycle();
    osDelay(100);
  }
}
...
void StartDefaultTask(void *argument)
{
   ...
   * ICM42688 独立诊断模式
   *
   * 只运行 v1.0 曾验证成功的 ImuDebug 路径；不初始化电机、编码器、循迹
   * 传感器或主控算法，用于排除其他模块和初始化顺序对 SPI2 的影响。
   * ======================================================================== */
  printf("\r\n");
  printf("============================================================\r\n");
  printf(" ICM42688 STANDALONE DIAGNOSTIC - MOTORS WILL NOT BE STARTED\r\n");
  printf("============================================================\r\n");

  while (!ImuDebug_Init()) {
      printf("\r\n[IMU-DIAG] Init failed; retrying the exact path in 3 seconds...\r\n");
      osDelay(3000);
  }

  printf("[IMU-DIAG] Init passed; streaming sensor data every 500 ms.\r\n");
  for (;;) {
      ImuDebug_Run(500);
      osDelay(10);
  }

#elif defined(TEST_MODE_IR_CALIBRATION)
  /* ========================================================================
   * IR传感器一步校准模式
   * ======================================================================== */
  ...
  printf("\r\n");
  printf("╔════════════════════════════════════════════════════════════════╗\r\n");
  printf("║       H Problem Tasks 2-6 - STM32F407 Competition App         ║\r\n");
  printf("╚════════════════════════════════════════════════════════════════╝\r\n");
  printf("\r\n");

  if (!CompetitionTasks_Init()) {
      printf("[FATAL] CompetitionTasks_Init failed — motors halted\r\n");
      for (;;) { osDelay(1000); }
  }
  ...
  printf("[Competition] K1=previous, K2=next, K3=confirm/start; "
         "K4/K5 disabled\r\n");
