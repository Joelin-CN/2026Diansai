...
基于 MSPM0G3507、FreeRTOS、四电机驱动、四路正交编码器、ICM42688 和 8 路 IR 循迹（UART1）的控制器固件。
...
| Flash | 128 KiB；当前镜像 ROM 33,080 bytes |
| SRAM | 32 KiB；当前镜像 RW 9,992 bytes |
...
| I2C0 | PA0 SDA、PA1 SCL，100 kHz，仅 3.3 V OLED（MCP23017 已移除） |
...
| UART1 | PA8/PA9，115200，8 路 IR 循迹模块 |
...
验收输出：`Host tests: PASS`。当前共有 6 个主机测试程序（`test_control_app` 因预先存在的协调器测试桩与测试期望漂移暂跳过，详见 `logs/ir_tracker_WORK_SUMMARY_20260728.md`）。
...
当前 HEX 文件约 90.93 KiB；实际烧入 Flash 的有效镜像为 33,080 bytes。
...
- 6 个主机测试程序全部通过（`test_control_app` 暂跳过，详见 `logs/ir_tracker_WORK_SUMMARY_20260728.md`）。
...
- UART1 已承载 8 路 IR 循迹模块的帧协议与 ISR→任务数据流（`UART1_IRQHandler`→`IrUartSensor_RxByte`→`ReadIr`）；UART0/3 已初始化，但仍没有应用层通信任务、协议、缓冲和收发数据流。
